...
describe Searchables::CinerVideo do
  subject(:searchable) { ciner_video }

  let(:ciner_video) { create(:ciner_video) }
  ...
  describe 'search methods' do
    ...
    describe '#search_link' do
      let(:expected_search_link) do
        searchable.url_helper.admin_ciner_videos_path(ciner_video: searchable.id)
      end
    ...
    describe '#search_title' do
      let(:expected_search_title) { ciner_video.title }

      it { expect(searchable.search_title).to eq expected_search_title }
    end

    describe '#search_description' do
      let(:expected_search_description) { ciner_video.synopsis }
